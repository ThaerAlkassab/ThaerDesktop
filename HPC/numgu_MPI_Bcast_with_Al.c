...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        while (inp != gen)
        {
            ...
            MPI_Recv(&inp, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &Stat);
            c++;
            if (inp > gen)
            {
            ...
            else if (inp < gen)
            {
                ...
                rep = 0;
            MPI_Send(&rep, 1, MPI_INT, Stat.MPI_SOURCE, 0, MPI_COMM_WORLD);
        }
    ...
    else
    {
        ...
        while (rep != 0)
        {
            guess = round(min + (max - min) / ((float)size-1)); 
            printf("R%d: I am guessing %d.\n", rank, guess);
            MPI_Send(&guess, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
            MPI_Recv(&rep, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (rep > 0)
                max = guess - 1;
            else if (rep < 0)
                min = guess + 1;
