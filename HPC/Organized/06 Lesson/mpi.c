...
void printMatix(myInt N, myFloat* data, myInt s_start, myInt s_end, int rank, int ranks) {
    myInt i, j;
    char c = 'x';
    MPI_Status Stat;
    if (rank > 0) {
        MPI_Recv(&c, 1, MPI_CHAR, rank-1, 1, MPI_COMM_WORLD, &Stat);
    }
    
    myInt nrows = s_end - s_start;
    for(i = 0; i < nrows; i++) {
        printf("%d:%d ", rank, i+s_start);
        for( j = 0; j < N; j++ ) {
    ...
    if(rank < ranks - 1) {
        MPI_Send( &c, 1, MPI_CHAR, rank+1, 1, MPI_COMM_WORLD );
    }   
}
...
int main(int argc, char ** argv) {
    ...
    printMatix(N, data, s_start, s_end, rank, ranks);
